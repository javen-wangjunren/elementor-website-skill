...
> 用 AI 规划、设计并高还原实现可维护的 Elementor 自定义 Widget，完成一套可信的 B2B 企业站。
...
这不是一个下载后即可安装的通用 Widget 插件，而是一套帮助你为**当前网站项目**完成页面设计、开发自定义 Elementor Widget，并生成对应插件的 Skills。你可以把它理解为：先在本地完成页面与 Elementor 模块的设计和开发，再将生成的插件安装到 WordPress 中使用。

最简单的使用路径是：

1. 下载或克隆这个仓库，将根目录下的 7 个 Skill 文件夹复制到 `~/.agents/skills/elementor/`。
2. 新建一个本地项目工作区，例如 `my-b2b-site/`；它不需要是 WordPress 根目录。
3. 调用 `elementor-site-team-manager`，告诉它你的公司资料、计划页面和目标；总控会判断当前该走内容、视觉、HTML 还是 Widget 实现。
4. 第一次需要自定义 Widget 时，`elementor-site-initialize` 会在你确认的本地路径创建插件骨架；后续由设计、HTML 和 Widget Skills 按确认结果逐步完成页面与组件。
...
你得到的不是一组固定的 Widget 模板，而是一条从项目初始化、内容与视觉梳理，到自定义 Elementor Widget 落地的完整路径。跟着它走，你可以：
...
AI 可以很快做出一个“看起来完整”的网页，但未经确认的判断也可能被一路带进代码，让最终页面带着明显的“AI 味”。常见原因包括：
...
<p align="center">
  <img src="./assets/readme/workflow.svg" width="100%" alt="从 Company Intake、Page Content、Design System、UI Architecture Map、视觉方向与 HTML Review 到 Elementor Widget 的完整工作流">
</p>
...
如果页面比较复杂、局部方向还需要逐步确认，或者模块需要跨页面复用，`website-html-prototyper` 可以一次只制作并审查一个模块的 HTML，确认后再拼成整页。
