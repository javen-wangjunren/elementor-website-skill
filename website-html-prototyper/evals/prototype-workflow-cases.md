...
| 10 | 完整页面的 Map、Overview、Segment 均已确认 | 可直接生成并精调整页 HTML，不强制创建独立模块文件。 |
...
| 57 | 完整页面只有已确认 Map，没有确认 Overview/Segment | 不生成 HTML，返回 `website-ui-architect` 完成视觉确认。 |
| 58 | 简单模块有完整方案、无歧义强参考和用户确认 | 允许不单独生图，直接生成模块 HTML。 |
| 59 | Overview 与 Segment 局部不一致 | 局部采用 Segment，整页 Surface 与节奏参考 Overview。 |
...
| 70 | 首页 fixture 同时提供 Map、`page-overview.png`、`page-segment-01-s1-s4.png` 与 `page-segment-02-s5-s8.png` | 业务与结构先服从 Map；S1–S4 局部构图服从 Segment 01，S5–S8 服从 Segment 02；Overview 只控制整页 Surface 与节奏。 |
